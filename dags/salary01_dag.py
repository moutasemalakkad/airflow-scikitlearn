...
def salary01_dag():
    ...
    def train_model(X_train, y_train, X_test, y_test):
        ...
        filename = '/usr/local/airflow/finalized_model.sav'
        pickle.dump(regression_model_sklearn, open(filename, 'wb'))
    ...
    @task
    def evaluate(regression_model_sklearn, X_test, y_test):
        regression_model_sklearn = pickle.load(open(regression_model_sklearn, 'rb'))
    ...
    train_model = train_model(featurized['X_train'], featurized['y_train'], featurized['X_test'], featurized['y_test'])
    evaluate(train_model, featurized['X_test'], featurized['y_test'])


salary01 = salary01_dag()
